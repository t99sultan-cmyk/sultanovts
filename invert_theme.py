...
# Split the file at the offer section
split_marker = "{/* OFFER"
...
# Replacements for Part 1 (Info -> Light Theme)
reps1 = [
    # General bg
    ('bg-[#0A0705]', 'bg-white'),
    ...
    ('border-[#FBBF24]', 'border-[#FBBF24]'), # Keep gold
...
for r in reps1:
    part1 = part1.replace(r[0], r[1])

# Fix for Guarantee Box readability specifically
# It's currently red-ish in the background. If we change it to light theme, we need to ensure the text contrast is amazing.
part1 = part1.replace('bg-red-500/10', 'bg-red-100')
part1 = part1.replace('bg-red-500/20', 'bg-red-200')
part1 = part1.replace('border-red-500/30', 'border-red-500')
part1 = part1.replace('text-red-400', 'text-red-600')

# Also, logos at the bottom:
part1 = part1.replace('bg-white/5', 'bg-black/5')
part1 = part1.replace('hover:bg-white/10', 'hover:bg-black/10')
part1 = part1.replace('hover:text-gray-900', 'hover:text-black') # was hover:text-white
part1 = part1.replace('border-black/10', 'border-black/5')

# Offer Section (Sales -> Dark Theme)
reps2 = [
...
# Part 2: Form inversion (Light -> Dark)
...
for r in reps3:
    form_content = form_content.replace(r[0], r[1])

with open(form_path, "w", encoding="utf-8") as f:
    f.write(form_content)

print("Updated MultiStepForm.tsx")
